...
export const GalleryApp: React.FC<GalleryAppProps> = ({ 
  ...
  const handleTabChange = (tab: 'gallery' | 'music' | 'timeline') => {
    if (tab === 'gallery' && !gallery.settings.allowComments) {
      return; // Don't switch if disabled
    }
    if (tab === 'music' && !gallery.settings.spotifyIntegration) {
      return;
    }
    setActiveTab(tab);
  };
  ...
  useEffect(() => {
    if (!gallery.settings.allowComments && activeTab === 'gallery') {
      setActiveTab('timeline');
    }
    if (!gallery.settings.spotifyIntegration && activeTab === 'music') {
      setActiveTab('timeline');
    }
  }, [gallery.settings, activeTab]);
    ...
    <div className={`min-h-screen transition-all duration-500 ${
      ...
      <div className="max-w-md mx-auto px-2 sm:px-0">
        ...
        <TabNavigation 
          ...
          isDarkMode={isDarkMode}
          galleryEnabled={true}
          musicWishlistEnabled={gallery.settings.spotifyIntegration}
        />
